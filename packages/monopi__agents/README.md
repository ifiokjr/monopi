...
These templates are consumed by `@monopi/cli` and the monopi installer. Most users should install the full bundle:
...
The monopi CLI (`pnpm monopi-cli`) lets you pick a template during setup. The template is copied to your project as `AGENTS.md`, where pi reads it on startup.

To manually use a template:

1. Copy the relevant markdown file from `packages/monopi__agents/agents/<template>.md`
