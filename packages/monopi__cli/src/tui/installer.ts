import { EXTENSIONS } from "@monopi/core";
...
export async function runInstaller(deps: InstallerDeps = defaultInstallerDeps): Promise<void> {
	...
	stdout.write(chalk.bold.cyan("║     monopi Interactive Installer     ║\n"));
	stdout.write(chalk.bold.cyan("╚══════════════════════════════════════╝\n"));
	stdout.write("\n");
	...
	if (installedVersion) {
		...
		stdout.write(`${chalk.dim("No existing monopi installation detected")}\n`);
	}
	stdout.write("\n");
	...
	stdout.write("\n");
	stdout.write(chalk.green.bold("✔ monopi installed successfully!\n"));
