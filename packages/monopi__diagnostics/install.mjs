...
export const PACKAGE_NAME = "@monopi/diagnostics";
...
Usage:
  npx @monopi/diagnostics            Install globally
  npx @monopi/diagnostics --local    Install into project .pi/settings.json
  npx @monopi/diagnostics --remove   Remove from pi
	...
	if (opts.remove) {
		log(
			result.status === "already-removed"
				? "\n✅ @monopi/diagnostics is already absent from pi."
				: "\n✅ Removed @monopi/diagnostics from pi.",
		);
	} else {
		log(
			result.status === "already-installed"
				? "\n✅ @monopi/diagnostics is already installed in pi."
				: "\n✅ Installed @monopi/diagnostics into pi. Restart pi to load it.",
		);
	}
