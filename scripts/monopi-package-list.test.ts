...
} from "../packages/monopi__monopi/bin/package-list.mjs";
import {
	EXPERIMENTAL_PACKAGES,
	INSTALLER_PACKAGES,
	SWITCHER_PACKAGES,
} from "../packages/monopi__monopi/bin/package-list.mts";

const scriptsDir = path.dirname(fileURLToPath(import.meta.url));
const repoRoot = path.resolve(scriptsDir, "..");
const installerPath = path.join(repoRoot, "packages", "monopi__monopi", "bin", "monopi.mjs");

afterEach(() => {
	vi.restoreAllMocks();
});

describe("monopi package list", () => {
	...
	it("loads the installer entrypoint with the package list import", async () => {
		...
		expect(log).toHaveBeenCalledWith(expect.stringContaining("@monopi/background-tasks"));
	});
});
