<p>
  <img src="banner.png" alt="@monopi/subagents" width="1100">
</p>
...
Or install it as part of the full monopi bundle:
...
```bash
npx @monopi/subagents
npx @monopi/subagents --local
```

To remove:

```bash
npx @monopi/subagents --remove
```
...
To inspect why a delegated startup pick resolved the way it did, use `@monopi/adaptive-routing`'s `/route why <category|role-override> [task text]` command.
...
`@monopi/subagents` reads optional JSON config from `~/.pi/agent/extensions/subagent/config.json`.
...
```typescript
import { runDynamicAgent } from "@monopi/subagents/dynamic-agent.js";
