...
describe("setupAdaptiveRouting", () => {
	...
	it("installs missing packages with an explicit scope, configures routing, and re-renders the dashboard", async () => {
		dashboardMocks.detectOptionalRoutingPackages
			.mockReturnValueOnce([missingPackage("@monopi/provider-ollama", "Ollama provider package")])
			.mockReturnValueOnce([])
			.mockReturnValueOnce([]);
		dashboardMocks.suggestOptionalRoutingPackages
			.mockReturnValueOnce(["@monopi/provider-ollama"])
			.mockReturnValueOnce([]);
		promptState.confirm.push(true, true);
		promptState.multiselect.push(["@monopi/provider-ollama"]);
		...
		expect(packageMocks.installPiPackages).toHaveBeenCalledWith(["@monopi/provider-ollama"], "project");
		...
		expect(dashboardMocks.detectOptionalRoutingPackages).toHaveBeenNthCalledWith(2, undefined, [
			{ packageName: "@monopi/provider-ollama", scope: "project" },
		]);
	...
	it("shows a note instead of installing packages when pi is missing", async () => {
		...
		dashboardMocks.detectOptionalRoutingPackages.mockReturnValue([missingPackage("@monopi/provider-ollama")]);
	...
	it("skips installation when the user declines and returns the current config when not reconfiguring", async () => {
		...
		dashboardMocks.detectOptionalRoutingPackages.mockReturnValue([missingPackage("@monopi/provider-ollama")]);
		dashboardMocks.suggestOptionalRoutingPackages.mockReturnValue(["@monopi/provider-ollama"]);
