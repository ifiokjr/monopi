...
function createDelegatedTestConfig() {
	return {
		...DEFAULT_ADAPTIVE_ROUTING_CONFIG,
		delegatedRouting: {
			...DEFAULT_ADAPTIVE_ROUTING_CONFIG.delegatedRouting,
			categories: {
				...DEFAULT_ADAPTIVE_ROUTING_CONFIG.delegatedRouting.categories,
				"quick-discovery": {
					candidates: ["google/gemini-2.5-flash", "openai/gpt-5-mini"],
					preferFastModels: true,
					preferredProviders: ["google", "openai"],
					taskProfile: "planning" as const,
				},
			},
		},
		delegatedModelSelection: {
			...DEFAULT_ADAPTIVE_ROUTING_CONFIG.delegatedModelSelection,
...
describe("delegated runtime helpers", () => {
	...
	it("builds merged delegated policies from category defaults and role overrides", () => {
		...
		const result = buildDelegatedSelectionPolicy({
			config: createDelegatedTestConfig(),
