...
```bash
pi install npm:@monopi/diagnostics
```
...
`@monopi/diagnostics` and `tool-metadata` share timestamp helpers, keeping prompt-level and tool-level timing displays consistent. Diagnostics operates at the prompt level (start-to-finish), while tool-metadata operates at the individual tool-call level.
