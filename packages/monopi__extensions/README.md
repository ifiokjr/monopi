...
- bg-process (powered by `@monopi/background-tasks`)
- usage-tracker
- scheduler
- btw / qq
- watchdog / safe-mode
...
`bg-process` now delegates to the richer `@monopi/background-tasks` runtime, so the core bundle also gets `/bg`, `Ctrl+Shift+B`, and the `bg_task` tool.
...
The `answer` extension extracts questions from the last LLM response and presents them in an interactive Q&A overlay powered by `@monopi/shared-qna`.
...
The `worktree` extension adds centralized git worktree awareness for monopi:
