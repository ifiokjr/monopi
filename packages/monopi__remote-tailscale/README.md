...
`@monopi/remote-tailscale` brings the original `pi-remote` workflow into monopi, but reuses the shared `@monopi/web-server` package for the HTTP and WebSocket transport layer.
...
1. creates a `PiWebServer` via `@monopi/web-server`
...
```bash
pnpm exec vitest run --config packages/monopi__remote-tailscale/vitest.config.ts --coverage
```
