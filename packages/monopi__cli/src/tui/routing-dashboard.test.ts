import type { ProviderConfig } from "@monopi/core";
...
describe("detectOptionalRoutingPackages", () => {
	it("maps installed scopes and selected package state", () => {
				...
				packageNames.map((packageName) => ({
					...
					scope:
						packageName === "@monopi/provider-ollama"
							? "user"
							: packageName === "@monopi/provider-cursor"
								? "project"
								: "none",
				})),
		...
		expect(packages.find((pkg) => pkg.packageName === "@monopi/provider-ollama")).toMatchObject({
			installed: true,
			scope: "user",
			selected: false,
		});
		expect(packages.find((pkg) => pkg.packageName === "@monopi/provider-cursor")).toMatchObject({
			installed: true,
			scope: "project",
		});
		expect(packages.find((pkg) => pkg.packageName === "@monopi/adaptive-routing")).toMatchObject({
			installed: false,
			selected: true,
...
describe("suggestOptionalRoutingPackages", () => {
	it("suggests routing and provider packages based on config and providers", () => {
		...
		).toEqual(["@monopi/adaptive-routing", "@monopi/provider-ollama", "@monopi/provider-cursor"]);
	});
...
describe("buildRoutingDashboard", () => {
	it("surfaces packages, providers, delegated assignments, and effective routing", () => {
		const dashboard = buildRoutingDashboard({
			...
			packageStates: [
				{
					packageName: "@monopi/adaptive-routing",
		...
		expect(dashboard).toContain("install with pi install npm:@monopi/provider-cursor");
