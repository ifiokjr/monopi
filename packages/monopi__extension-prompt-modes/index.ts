...
function computeModesPatch(base: ModesFile, next: ModesFile, includeCurrentMode: boolean): ModesPatch | null {
	...
	for (const k of keys) {
		...
		for (const f of fields) {
			...
			const bv = b[f];
			if (av !== bv) {
				(diff as Record<string, unknown>)[f] = bv === undefined ? null : bv;
			}
		}
...
interface ModeTheme {
	getFgAnsi(color: string): unknown;
	fg(color: string, text: string): string;
	getThinkingBorderColor(level: string): (text: string) => string;
}

function getModeBorderColor(theme: ModeTheme, pi: ExtensionAPI, mode: string): (text: string) => string {
	...
	if (spec?.color) {
		try {
			...
			return (text: string) => theme.fg(spec.color, text);
		} catch {
...
function collectUserPromptsFromEntries(entries: Array<SessionEntryLike>): PromptEntry[] {
...
async function loadPromptHistoryForCwd(cwd: string, excludeSessionFile?: string): Promise<PromptEntry[]> {
	...
	for (const file of sortedFiles) {
		...
		for (const line of lines) {
			...
			try {
				entry = JSON.parse(line) as SessionEntryLike;
			} catch {
				continue;
			}
			if (entry?.type !== "message") continue;
