...
Thank you for your interest in monopi! All forms of contributions are welcome.
...
**Every change must include a changeset.** This is enforced in CI on every push and PR. The only exception is generated `chore: release` commits from the release workflow.

```bash
pnpm change
```

Because this repo uses MonoChange with a lockstep `monopi-group`, changeset frontmatter must use **only** `monopi-group` as the key, not package names.
...
4. Create and commit a changeset: `pnpm change`
...
Use the [Issue templates](https://github.com/ifiokjr/monopi/issues/new/choose) and include:
